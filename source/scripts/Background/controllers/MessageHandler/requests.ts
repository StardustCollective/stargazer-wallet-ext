...
const handleTransparentRequest = async (message: Message, masterController: IMasterController) => {
  ...
  const provider = getInfuraProvider('testnet');
...
export const handleRequest = async (
  ...
  const activeWallet: KeyringWalletState | null = vault?.activeWallet
    ...
    try {
      ...
      return Promise.reject(
        new CustomEvent(message.id, {
          detail: e instanceof Error ? e.message : 'transparent-call-error',
        })
      );
    }
  }
