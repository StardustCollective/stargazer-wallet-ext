...
class RNEncryptor<T> {
  async encrypt(password: string, data: T): Promise<string> {
    ...
    const iv = (crypto as any).getRandomValues(new Uint8Array(16));
    const text = JSON.stringify(data);

    const encryptedData = AES.encrypt(text, this.getKey(password, salt)).toString();
  ...
  generateSalt(byteCount = 32): string {
    ...
    (crypto as any).getRandomValues(view);
